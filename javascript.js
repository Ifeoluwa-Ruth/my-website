function Message(){ 
    var messagetype = document.getElementById("name").value;
    document.getElementById("destination").value = messagetype;
    window.alert("Hi" +  " " +messagetype + ", \n\n We got you message.\n\n We will respond to you shortly.")
}

const product =[
       {
        id: 0,
        image: 'image/cake1.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },
       
       {
        id: 1,
        image: 'image/cake2.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 2,
        image: 'image/cake3.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 3,
        image: 'image/cake4.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 4,
        image: 'image/cake5.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 2,
        image: 'image/cake6.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 2,
        image: 'image/cake7.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 2,
        image: 'image/cake8.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 2,
        image: 'image/cake9.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 2,
        image: 'image/cake10.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 2,
        image: 'image/cake11.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       },

       {
        id: 2,
        image: 'image/cake12.jpg',
        title: 'Chocolate Cake',
        price: 3000,
       }
];
const categories = [...new Set(product.map((item)=>
    {return item}))]
    let i=0;
document.getElementById('root').innerHTML = categories.map((item)=>
{
    var {image, title, price} = item;
    return(
        `<div class='box'>
            <div class='img-box'>
                <img class='images' src=${image}></img>
            </div>
        <div class='bottom'>
        <p>${title}</p>
        <h2>$ ${price}.00</h2>`+
        "<button onclick='addtocart("+(i++)+")'>Add to cart</button>"+
        `</div>
        </div>`
    )
}).join('')

var cart =[];

function addtocart(a){
    cart.push({...categories[a]});
    displaycart();
}
function delElement(a){
    cart.splice(a, 1);
    displaycart();
}

function displaycart(){
    let j = 0, total=0;
    document.getElementById("count").innerHTML=cart.length;
    if(cart.length==0){
        document.getElementById('cartItem').innerHTML = "Your cart is empty";
        document.getElementById("total").innerHTML = "$ "+0+".00";
    }
    else{
        document.getElementById("cartItem").innerHTML = cart.map((items)=>
        {
            var {image, title, price} = items;
            total=total+price;
            document.getElementById("total").innerHTML = "$ "+total+".00";
            return(
                `<div class='cart-item'>
                <div class='row-img'>
                    <img class='rowimg' src=${image}>
                </div>
                <p style='font-size:12px;'>${title}</p>
                <h2 style='font-size: 15px;'>$ ${price}.00</h2>`+
                "<i class='fa-solid fa-trash' onclick='delElement("+ (j++) +")'></i></div>"
            );
        }).join('');
    }

    
}

         let checkout_modal = document.querySelector("#checkout_modal");
         let paymenttForm = document.querySelector("#pmtForm");
         let close = document.querySelector(".close");
         let overlay = document.querySelector(".overlay");

         checkout_modal.addEventListener("click",()=>{
            paymenttForm.style.opacity="1";
            paymenttForm.style.visibility="visible";
         });

         close.addEventListener("click",()=>{
            paymenttForm.style.opacity="0";
            paymenttForm.style.visibility="hidden";
         })


         function CopyText()
         {
             var messagetype = document.getElementById("total").value;
         
             document.getElementById("destination").value = messagetype;
         }